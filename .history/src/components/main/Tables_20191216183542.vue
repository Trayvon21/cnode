  ...
  methods: {
        Formate(val){
            ...
            if(Number(this.$dayjs(time).format("MM"))>1){
                ...
                return `${Number(this.$dayjs(time).format("DD"))-1}天以前`
            }else if(Number(this.$dayjs(time).format("hh"))>1){
                 return `${Number(this.$dayjs(time).format("hh"))-1}小时以前`
            }else if(Number(this.$dayjs(time).format("mm"))>1){
                 return `${Number(this.$dayjs(time).format("mm"))-1}分钟以前`
            }else if(Number(this.$dayjs(time).format("ss"))>1){
                 return `${Number(this.$dayjs(time).format("ss"))-1}秒以前`
            }
